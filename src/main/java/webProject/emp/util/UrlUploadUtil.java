...
import webProject.uploadEntity.DownloadWithRange;
 ...
 */
@Component
public class UrlUploadUtil {
	// 记录文件名
	ThreadLocal<String> fileNameNew = new ThreadLocal<String>();
	Logger logger = LogManager.getLogger(UrlUploadUtil.class.getName());
	...
	public synchronized boolean uploadFile(String downloadUrl, String filePath, String fileName, String localFileName)
		...
		try {
			...
			logger.info("下载文件为:" + fileNameNew.get());
			...
			BlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<>(8);// 队列中最大接收2个任务，超过2个则会抛异常
			...
			for (int i = 0; i < maximumPoolSize; i++) {
				...
				final DownloadWithRange download = new DownloadWithRange(downloadUrl, filePath, fileNameNew.get(),
						start, end);
			...
			logger.info("多线程创建异常:" + e.getMessage());
			flag = false;// 文件下载异常
		} finally {
		...
		if (!flag && file.exists()) {
			...
			logger.info("删除下载异常文件:" + fileNameNew.get() + ":" + mark);
			return false;
		}
		if (!threadFlag && file.exists()) {
			System.out.println("进入文件删除2,删除文件:" + file);
			logger.info("删除下载异常文件:" + fileNameNew.get() + ":" + file.delete());
			return false;
		}
		return true;
	}
