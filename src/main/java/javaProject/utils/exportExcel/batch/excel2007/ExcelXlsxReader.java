package javaProject.utils.exportExcel.batch.excel2007;
 ...
 * 本类用于读取excel2007
 * 	
 * 	DefaultHandler：处理器方法默认执行顺序
 * 
 * 		startDocument--》startElement--》characters--》endElement--》endDocument
 * 
 * 	实现默认处理器需要实现的方法，处理器会自动跟据顺序执行	
 * 
 * 	使用DefaultHandler解析excel可以有效地防止HSSFWorkbook、SXSSFWorkbook内存溢出的问题，可以解决大批量的excel解析
 * 
 * @author 56525
 *
 */
public class ExcelXlsxReader extends DefaultHandler {
	...
	*/
	...
	/**
	 * 解析到XML的结束标签触发此方法：自动触发，对上面characters返回的单元格的值进行处理
	 * 		标签说明
	 * 			<row>:行结束标签
	 * 			<c>:标签结束
	 * 			<v>:文本单元格结束标签
