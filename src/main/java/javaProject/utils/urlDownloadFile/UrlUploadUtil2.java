...
public class UrlUploadUtil2 {
	...
	public static void main(String[] args) throws IOException {
		...
		UrlUploadUtil2 uploadUtil = new UrlUploadUtil2();
		// 指定存放地址
	...
	public synchronized boolean uploadFile(String downloadUrl, String filePath, String fileName, String localFileName)
		...
		UtilUploadUtil2WithRange download = null;
		try {
			// 获取文件大小
			...
			System.out.println(fileNameNew.get() + "文件大小:" + fileSize);
			...
			long keepAliveTime = 1;// 当线程数大于核心时，此为终止前多余的空闲线程等待新任务的最长时间
			TimeUnit unit = TimeUnit.MINUTES;// 指明等待时间的量化值的单位
			// 保存到队列的任务数，当任务书达到队列最大时拒绝加入，抛异常等
			BlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<>(8);// 队列中最大接收8个任务，超过八个则会抛异常
			// 特别强调要使用new ThreadPoolExecutor创建，这样main方法和页面测试都没问题
			...
			for (int i = 0; i < maximumPoolSize; i++) {
				...
				download = new UtilUploadUtil2WithRange(downloadUrl, filePath, fileNameNew.get(), start, end);
